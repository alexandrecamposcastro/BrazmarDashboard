...
app.put("/api/cases/:id/timesheet/:tid", auth, async (req, res) => {
  try {
    const { atividade, horas, data, sigla } = req.body;
    if (!atividade || !horas) return res.status(400).json({ error: "atividade e horas obrigatórios" });
    const updated = await db.updateTimesheet(req.params.tid, req.params.id, { atividade, horas, data, sigla });
    res.json(updated);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post("/api/webhook/email", async (req, res) => {
  try {
    ...
    if (melhorCaso && melhorScore >= 60) {
      ...
      // Mescla summary via Gemini se o caso já tem um resumo anterior
      if (summary) {
        if (melhorCaso.summary && melhorCaso.summary.trim() && process.env.GEMINI_API_KEY) {
          try {
            const mergePrompt = `Você é um assistente da BRAZMAR Marine Services. Abaixo estão dois resumos do mesmo caso marítimo (navio "${vessel}"). Combine-os em um único resumo completo, preservando todas as informações relevantes de ambos sem repetir. Seja conciso.
...
RESUMO COMBINADO:`;
            const gBody = JSON.stringify({ contents: [{ parts: [{ text: mergePrompt }] }], generationConfig: { temperature: 0.2, maxOutputTokens: 800 } });
            const geminiRes = await new Promise((resolve, reject) => {
              const opts = { hostname: "generativelanguage.googleapis.com", path: "/v1beta/models/gemini-1.5-flash:generateContent?key=" + process.env.GEMINI_API_KEY, method: "POST", headers: { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(gBody) } };
              const req = https.request(opts, r => { let d = []; r.on("data", c => d.push(c)); r.on("end", () => resolve(JSON.parse(Buffer.concat(d).toString()))); });
              req.on("error", reject); req.write(gBody); req.end();
            });
            const merged = geminiRes.candidates?.[0]?.content?.parts?.[0]?.text;
            updates.summary = merged ? merged.trim() : summary;
          } catch(e) { console.error("Merge summary error:", e.message); updates.summary = summary; }
        } else {
...
app.get("/api/cases/:id/timesheet/export", auth, async (req, res) => {
  try {
    ...
    for (const e of entries) {
      const key = `${e.data||""}||${(e.sigla||"").toUpperCase()}`;
      if (mergedMap.has(key)) {
        const prev = mergedMap.get(key);
        prev.atividade = prev.atividade + " / " + (e.atividade||"");
        prev.horas = Number(prev.horas) + Number(e.horas);
      } else {
        mergedMap.set(key, { ...e, horas: Number(e.horas) });
      }
    }
    const mergedEntries = [...mergedMap.values()].sort((a,b)=>{
      // Ordena por data (DD/MM/YYYY → YYYY-MM-DD para comparar)
      const toISO = d => { if(!d) return ""; const p=d.split("/"); return p.length===3?`${p[2]}-${p[1]}-${p[0]}`:d; };
      return toISO(a.data).localeCompare(toISO(b.data));
    });

    const totalGeral = mergedEntries.reduce((s,e)=>s+Number(e.horas),0);
    const totaisSigla = {};
    ...
    const portoSigla = (caso.porto||"XXX").split(" ").map(w=>w[0]).join("").toUpperCase().slice(0,3);
    const refPart = caso.ref ? caso.ref : `XXXX.${anoAtual}.${portoSigla}`;
    const vesselClean = (caso.vessel||"VESSEL").toUpperCase();
    const sufixo = tipo === "pessoal" ? siglaLogado : "full timesheet";
    const filename = `BRAZMAR - ${refPart} - ${vesselClean} - ${sufixo}.docx`;

    // ── Upload automático para Dropbox: BRAZMAR - time sheets / REF - VESSEL
    try {
      const tsFolder = `${refPart} - ${vesselClean}`;
      await dropbox.uploadTimesheet(tsFolder, filename, buffer);
    } catch(dbErr) {
